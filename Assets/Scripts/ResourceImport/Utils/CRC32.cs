...
namespace Storm
{
    public class CRC32
    {
        const UInt32 CRC_NULL = 0xFFFFFFFF;
    ...
    public class CRC32deprecated : HashAlgorithm
    {
        uint[] table = new uint[256];
        const UInt32 CRC_NULL = 0xFFFFFFFF;
        public CRC32deprecated()
        {
            Initialize();
        }
        ...
        public override void Initialize()
        {
            ...
            for (int n = 0; n < 256; n++)
            {
                uint c = (uint)n;
                for (int k = 8; k != 0; k--)
                    c = Convert.ToBoolean(c) ? poly ^ (c >> 1) : c >> 1;
                table[n] = c;
            }
        }
        ...
        private UInt32 Code(string s)
        {
            UInt32 code = CRC_NULL;

            for (int i = 0; i < s.Length; i++)
            {
                var index = (byte) code ^ (byte) i;
                var ch = (char)code;
                //Debug.Log($"Length {s.Length} index {index} array {table.Length}");
                //Debug.Log($"Code=[{(UInt32) ch}], i=[{i}]");
                code = table[index] ^ (code >> 8);
